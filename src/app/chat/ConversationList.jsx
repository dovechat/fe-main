                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem' }}>
                    <span style={{ fontSize: '0.8125rem', color: '#4b5563', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {conv.last_message?.text || (conv.last_message?.media?.length > 0 ? (conv.last_message.media[0].mime_type?.startsWith('image/') ? 'Фото' : conv.last_message.media[0].mime_type?.startsWith('video/') ? 'Видео' : 'Файл') : '') conv.last_message.media[0].mime_type}
                    </span>
                    {conv.unread_count > 0 && (
